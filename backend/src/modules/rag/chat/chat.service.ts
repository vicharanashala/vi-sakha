...
export class ChatService {
   ...
   */
  async sendMessage(dto: SendMessageDto, deviceType?: 'mobile' | 'desktop' | 'unknown', userId?: string): Promise<{
    ...
    let dbCohort: string | undefined = dto.cohort;
    let dbEmail: string | undefined = dto.studentEmail;
    let dbName: string | undefined = dto.studentName;

    if (userId) {
      const dbUser = await this.usersService.findById(userId);
      if (dbUser) {
        dbCohort = dbCohort || dbUser.cohortName;
        dbEmail = dbEmail || dbUser.cohortEmail || dbUser.email;
        dbName = dbName || dbUser.name;
      }
    }
    ...
    if (dto.conversationId) {
      ...
      conversation = await this.conversationModel.create({
        studentId: dto.studentId || userId || 'anonymous',
        studentName: dbName || 'Student',
    ...
    // Call MCP pipeline with sanitized base64 data
    const cleanAttachments = dto.attachments?.map(a => {
      const base64Data = a.content.includes(';base64,') 
        ? a.content.split(';base64,')[1] 
        : a.content;
      return {
        name: a.name,
        type: a.type,
    ...
    const ragResult = await this.mcpService.processQuery({ 
      query: dto.content,
   ...
   */
  async *sendMessageStream(dto: SendMessageDto, deviceType?: 'mobile' | 'desktop' | 'unknown', userId?: string): AsyncGenerator<McpStreamEvent> {
    const startTime = Date.now();
    ...
    for await (const event of this.mcpService.processQueryStream({ 
      query: dto.content,
      attachments: cleanAttachments,
      cohort: dbCohort,
      studentEmail: dbEmail,
